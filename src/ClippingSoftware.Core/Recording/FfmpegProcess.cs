...
///
/// The other half of that same "child process outlives the C# call" risk: if cancellation (or a stream
/// read failure) throws past the awaits below, `using var process` alone only releases the .NET wrapper -
/// it never terminates the still-running OS process, which would otherwise keep running (and keep its
/// output file open/being written) in the background indefinitely. Both methods kill it in a `finally` if
/// it's still running by the time control leaves, whether that's the normal path (already exited, so this
/// is a no-op) or an exception.
/// </summary>
